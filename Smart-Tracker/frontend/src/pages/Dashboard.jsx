...
function Dashboard() {
  ...
  const pieChartDataForRender = useMemo(() => {
    ...
    // This represents the net result of income and expenses for the current month.
  ...
  // filteredPieData will now correctly include 'Balance' if it's positive
  const filteredPieData = pieChartDataForRender.filter(item => item.value > 0);
  // hasChartData condition might need adjustment if you *only* want to show chart if Income or Expense > 0
  // But generally, if there's any positive slice (Income, Expense, or resulting positive Balance), we can show the chart.
       ...
       <div className={styles.bottomRowContainer}>
            ...
            {loadingLimits ? (
              ...
              <>
                <div className={styles.limitList}>
                    {limits.slice(0, 4).map((limit) => { // Changed from 3 to 4
                        ...
                        const isOverspent = limit.exceeded || (limit.amount > 0 && spentAmount > limitAmount);
                        return (
                            <div key={limit._id} className={styles.limitItem}>
                                <div className={styles.limitDetails}>
                                    ...
                                    <div className={styles.limitAmounts}>
                                        ...
                                        <span className={styles.limitRemaining} style={{ color: isOverspent ? '#EF4444' : ((remainingAmount === 0 && limitAmount > 0) ? '#D97706' : '#10B981') }}>
                                            {isOverspent ? `Overspent: ${formatCurrency(Math.abs(remainingAmount))}` : `Left: ${formatCurrency(remainingAmount)}`}
                                        </span>
                                    </div>
                                </div>
                                {isOverspent && (<div className={styles.limitExceededMessage}>Limit Exceeded!</div> )}
                                ...
                                <div className={styles.limitTotal}>Set: {formatCurrency(limitAmount)}</div>
        ...
        <section className={`${styles.sectionBox} ${styles.goalsSection}`}>
            <div className={styles.sectionHeader}>
                ...
                <Link to="/goals" className={styles.seeAllButton}>See All Goals</Link>
            </div>
                    ...
                    {recentGoals.map(goal => (
                        <div key={goal._id} className={styles.goalItemDashboard}>
                            <div className={styles.goalItemHeaderDashboard}>
                                ...
                                <span className={styles.goalDescriptionDashboard}>{goal.description}</span>
                            </div>
                            ...
                            <div className={styles.goalDeadlineDashboard}>
                                Deadline: {formatDate(goal.targetDate, {month: 'short', day: 'numeric', year: '2-digit' })}
                            </div>
                        </div>
                    ))}
                </div>
        ...
        {/* --- END JSX FOR RECENT GOALS SECTION --- */}
